...
export default function ExpenseForm() {
  ...
  const { dispatch, state, balance } = useBudget();
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (Object.values(expense).includes("") || expense.amount <= 0) {
      setError("Todos los campos son obligatorios o el monto no es válido");
      return;
    }

    // Validar el saldo disponible
    if (( expense.amount - previousAmount ) > balance) {
      setError("El monto supera el saldo disponible");
      return;
    }

    // Agg o Actualizar un gasto
    if (state.editingId) {
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <input
        type="submit"
        className="bg-blue-600 cursor-pointer w-full py-2 uppercase text-white font-bold rounded-lg"
